...
const ToolbarButton = ({ icon, label, onClick, isActive }) => (
    <button
        onClick={onClick}
        className={`
      flex items-center justify-center {/* Added flex for icon centering */}
      h-10 w-10 {/* Fixed height and width for consistent sizing */}
...
const FormattingToolbar = ({ onFontChange, onSizeChange, onBoldToggle, onItalicToggle, onUnderlineToggle, onDownloadPdf,
    ...
    return (
        <div className="flex flex-wrap items-center gap-2 p-3 bg-zinc-900 rounded-lg shadow-inner mb-6">
            {/* Font Selector */}
            <select
                value={selectedFont}
            ...
            <select
                ...
                className="
          bg-zinc-800 text-gray-200 rounded-md py-2 h-10 {/* Added h-10 for consistent height */}
          focus:outline-none 
          cursor-pointer
        "
            ...
            {/* Bold Button - Using a simple icon for now, actual 'B' might need custom SVG */}
            <ToolbarButton
            ...
            />
            {/* Italic Button - Using a generic icon for now */}
            <ToolbarButton
                icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
            ...
            {/* Underline Button - Using a generic icon for now */}
            <ToolbarButton
                icon={<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
